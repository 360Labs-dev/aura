...
"#,
    ...
    fn generate_js(&mut self) {
        ...
        self.js
            .push_str("let _actionRegistry = Object.create(null);\n");
        self.js.push_str("let _actionId = 0;\n");
        self.js.push_str("function _registerAction(fn) {\n");
        self.js.push_str("  const id = `action-${++_actionId}`;\n");
        self.js.push_str("  _actionRegistry[id] = fn;\n");
        self.js.push_str("  return id;\n");
        self.js.push_str("}\n\n");
        self.js.push_str("function _invokeAction(id) {\n");
        self.js.push_str("  const action = _actionRegistry[id];\n");
        self.js.push_str("  if (typeof action === 'function') {\n");
        self.js.push_str("    action();\n");
        self.js.push_str("  }\n");
        self.js.push_str("}\n\n");
        ...
        self.js
            .push_str("  document.querySelectorAll('[data-action-id]').forEach(el => {\n");
        self.js.push_str("    const id = el.dataset.actionId;\n");
        self.js.push_str("    el.onclick = event => {\n");
        self.js.push_str("      event.preventDefault();\n");
        self.js.push_str("      _invokeAction(id);\n");
        self.js.push_str("    };\n");
        self.js.push_str("  });\n");
    ...
    fn view_to_js_template(&mut self, view: &HIRView, depth: usize) -> String {
        ...
        match view {
            ...
            HIRView::Button(button) => {
                ...
                let onclick = self.action_to_js(&button.action);
                let action_id = format!("${{_registerAction(() => {{ {} }})}}", onclick);
                let style_class = match button.style {
                    ButtonStyle::Icon => " icon",
                    ButtonStyle::Outline => " outline",
                    ButtonStyle::Ghost => " ghost",
                ...
                format!(
                    "{}<button class=\"aura-button{}{}\" data-action-id=\"{}\">{}</button>\n",
                    pad,
                    style_class,
                    if cls.is_empty() {
                        String::new()
    ...
    fn expr_to_js(&self, expr: &HIRExpr) -> String {
        match expr {
            HIRExpr::StringLit(s) => {
                if s.contains('{') && s.contains('}') {
                    // String interpolation: "Hello {name}" → `Hello ${state.name}`
  ...
  screen Main
    ...
    action doStuff
      return",
        ...
        assert!(
            output.js.contains("data-action-id"),
            "JS missing action binding"
        );
        assert!(
            output.js.contains("_registerAction"),
            "JS missing action registry"
        );
    ...
    #[test]
    fn test_loop_actions_capture_local_values() {
        let output = compile_source(
            "\
app Test
  model Todo
  ...
  screen Main
    ...
    action remove(todo: Todo)
      todos = todos.remove(todo)",
        ...
        assert!(
            output.js.contains("data-action-id"),
            "Expected action id binding in rendered button"
        );
    }
